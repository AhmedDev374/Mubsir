...
MUBSIR - مبصر is a browser-only SvelteKit 5 static application. Routes are finite (`/`, `/read?document=…`, and `/settings`) because document identifiers live in IndexedDB rather than the filesystem router.
