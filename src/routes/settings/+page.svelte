<script lang="ts">
	...
	let localVoiceStatus = $derived(
		!llmState.installed
			? 'Local model required — download/install model'
			: !appState.installedModels.includes(appState.selectedModelId)
				? 'Local speech model required — install voice model'
				: 'Local models ready'
	);
	...
	async function copyDiagnostics(): Promise<void> {
		try {
			...
			appState.errorMessage = 'This browser did not allow MUBSIR - مبصر to copy the diagnostic report.';
		}
	}
	...
	async function factoryReset(): Promise<void> {
		if (
			!confirm(
				'Erase ALL MUBSIR - مبصر data on this device? Documents, generated audio, downloaded models, API keys, and settings will be gone. This cannot be undone.'
			)
		)
			return;
		resetting = true;
...
<svelte:head>
	<title>Settings — MUBSIR - مبصر</title>
</svelte:head>
			...
			<header class="section-title">
				<div>
					...
					<p>
						...
						The local provider stays on-device; OpenAI remains optional for cloud voice.
					</p>
				</div>
				<span class="runtime-state" class:ready={providersState.voiceProvider === 'local' || providersState.hasKey('openai')}>
					<span></span>{providersState.voiceProvider === 'local' ? 'Local ready' : providersState.hasKey('openai') ? 'Ready' : 'Provider required'}
				</span>
			</header>

			<div class="assistant-group" role="group" aria-label="Voice provider selection">
				<span class="assistant-group-label">Provider</span>
				<div class="engine-models assistant-grid">
					<button
						type="button"
						class="engine-model"
						class:selected={providersState.voiceProvider === 'local'}
						aria-pressed={providersState.voiceProvider === 'local'}
						onclick={() => void providersState.setVoiceProvider('local')}
					>
						<strong>Local</strong>
						<small>Recommended · voice and docs stay on this device</small>
					</button>
					<button
						type="button"
						class="engine-model"
						class:selected={providersState.voiceProvider === 'openai'}
						aria-pressed={providersState.voiceProvider === 'openai'}
						onclick={() => void providersState.setVoiceProvider('openai')}
					>
						<strong>OpenAI</strong>
						<small>Cloud realtime · keep the existing API key flow</small>
					</button>
				</div>
			</div>

			{#if providersState.voiceProvider === 'local'}
				<div class="assistant-local-status" role="status">
					<strong>{localVoiceStatus}</strong>
					<span>
						STT: Faster-Whisper ({'tiny'} default, CPU INT8) · LLM:
						{llmState.selectedModel.label} ({llmState.selectedModel.sizeMb} MB) · TTS:
						Supertonic 3 ({installed ? 'installed' : 'not installed'})
					</span>
					{#if !llmState.installed}
						<small>Install the selected local LLM from the LLM settings before asking a question.</small>
					{/if}
					{#if !appState.installedModels.includes(appState.selectedModelId)}
						<small>Install the local speech model from Voice settings before expecting audio playback.</small>
					{/if}
				</div>
			{/if}
		...
		<section class="settings-section" aria-labelledby="llm-behavior-title">
			<header class="section-title">
				<div>
					...
					<p>How MUBSIR - مبصر uses the language model while you read.</p>
				</div>
			</header>
	...
	{:else if activeSection === 'appearance'}
		...
		<section class="settings-section" aria-labelledby="font-title">
			...
			<div class="appearance-preview" aria-hidden="true">
				<p>
					MUBSIR - مبصر reads your documents in this face — <em
		...
		<section class="settings-section" aria-labelledby="storage-title">
			<header class="section-title">
				<div>
					...
					<p>MUBSIR - مبصر’s private storage area on this device.</p>
				</div>
