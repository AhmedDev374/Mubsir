...
<svelte:head>
	...
	<title>MUBSIR - مبصر — local document listening</title>
</svelte:head>

<a class="skip-link" href="#main-content">Skip to content</a>

	<header class="app-header" class:sidebar-collapsed={sidebarCollapsed} aria-label="MUBSIR - مبصر header">
	<div class="app-brand-slot">
		<a class="brand" href={homeHref} aria-label="MUBSIR - مبصر library" onclick={closeNavigation}>
			<span class="brand-mark" aria-hidden="true">
				<BrandMark active={player.isPlaying || player.isBuffering} />
			</span>
			<span class="brand-name">MUBSIR - مبصر</span>
		</a>
		<button
		...
		<div class="reader-commandbar">
			...
			<div class="reader-commandbar-actions">
				...
				<a
					...
					target="_blank"
					rel="noreferrer"
					aria-label="Open MUBSIR - مبصر on GitHub"
					title="Open MUBSIR - مبصر on GitHub"
				>
...
<div class="app-shell" class:sidebar-collapsed={sidebarCollapsed} class:reader-mode={isReader}>
	<aside
		...
		aria-label="MUBSIR - مبصر navigation"
	>
		...
		<nav class="utility-nav" aria-label="MUBSIR - مبصر settings">
			<a
				class="nav-link"
...
{#if updateAvailable && !updateDismissed}
	<div class="toast update-toast" class:stacked={Boolean(appState.errorMessage)} role="status">
		<div>
			<strong>A MUBSIR - مبصر update is ready</strong>
			<p>
				{runtimeBusy
					? 'Finish or pause the current voice work before reloading.'
					: 'Reload when you are ready. MUBSIR - مبصر will never refresh itself.'}
			</p>
		</div>
...
{#if appState.importing}
	<div class="toast import-toast" role="status" aria-live="polite" aria-busy="true">
		<div class="import-toast-copy">
			<strong><Icon icon={LoaderCircle} class="spin" size={14} /> Processing document</strong>
			<p>{appState.statusMessage}</p>
			{#if appState.importProgress?.page && appState.importProgress.pageCount}
				<div class="import-progress-meta">
					<span>{appState.importProgress.page} / {appState.importProgress.pageCount} pages</span>
					<span>{Math.round((appState.importProgress.page / appState.importProgress.pageCount) * 100)}%</span>
				</div>
				<div class="import-progress-track" aria-hidden="true">
					<i
						style:width={`${Math.round((appState.importProgress.page / appState.importProgress.pageCount) * 100)}%`}
					></i>
				</div>
			{:else}
				<div class="import-progress-track indeterminate" aria-hidden="true"><i></i></div>
			{/if}
		</div>
		<button class="icon-button" type="button" aria-label="Cancel import" onclick={() => appState.cancelImport()}>
			<Icon icon={X} size={17} />
		</button>
	</div>
{/if}
