<script lang="ts">
	...
	function friendlyError(message?: string): string {
		...
		if (/out of memory|allocation|memory/i.test(message))
			return 'This browser ran out of working memory. Close other tabs, reopen MUBSIR - مبصر, and resume the installation.';
		return message;
	}
	...
	<header class="setup-heading">
		...
		<div class="setup-copy">
			<h1 id={compact ? 'reader-model-setup-title' : 'model-setup-title'}>
				{compact ? 'Finish setting up MUBSIR - مبصر' : 'Choose how MUBSIR - مبصر reads.'}
			</h1>
			<p>
				{compact
