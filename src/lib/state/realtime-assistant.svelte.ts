...
import {
	connectLocalVoiceService,
	LocalVoiceError,
	type LocalVoiceChannel
} from '$lib/services/local-voice';
import { getLlmHost } from '$lib/services/llm/llm-client';
...
export class RealtimeAssistantState {
	...
	private channel?: RealtimeChannel | LocalVoiceChannel;
	private microphone?: MediaStream;
	private audio?: HTMLAudioElement;
	private localAudioContext?: AudioContext;
	private localAudioSource?: AudioBufferSourceNode;
	...
	private async start(
		...
		try {
			...
			const usingLocal = providersState.voiceProvider === 'local';
			...
			if (usingLocal) {
				this.channel = await connectLocalVoiceService({
					onEvent: (event) => this.handleEvent(event),
					onClosed: () => {
						if (this.active) this.stop('The local voice session ended.');
					},
					signal: abort.signal
				});
				this.channel.send({
					type: 'session.start',
					model: providersState.realtimeModelId,
					voice: providersState.realtimeVoice,
					instructions: this.context.instructions,
					assistant_id: 'mubsir-local'
				});
	...
	private setMicrophoneOpen(open: boolean): void {
		...
		if (open) this.startLocalCapture();
		else this.stopLocalCapture(true);
	}

	private startLocalCapture(): void {
		if (!(this.channel && 'sendAudio' in this.channel) || !this.microphone || this.captureProcessor) return;
		try {
			...
			processor.onaudioprocess = (event) => {
				...
				this.captureChunks.push(new Float32Array(event.inputBuffer.getChannelData(0)));
			};
			...
			if (import.meta.env.DEV) console.debug('[local voice] microphone started', context.sampleRate);
		} catch (error) {
			if (import.meta.env.DEV) console.debug('[local voice] microphone capture failed', error);
		}
	}

	private stopLocalCapture(sendAudio: boolean): void {
		const processor = this.captureProcessor;
		...
		if (sendAudio && this.captureChunks.length && this.channel && 'sendAudio' in this.channel) {
			const length = this.captureChunks.reduce((total, chunk) => total + chunk.length, 0);
			...
			this.channel.sendAudio(samples, this.captureSampleRate);
			if (import.meta.env.DEV) console.debug('[local voice] microphone stopped', samples.length);
		}
		this.captureChunks = [];
	...
	private handleEvent(event: Record<string, unknown>): void {
		...
		switch (type) {
			case 'session.ready':
				if (import.meta.env.DEV) console.debug('[local voice] session ready', event.device, event.model);
				break;
			case 'transcription.final':
				void this.handleLocalTranscript(String(event.text ?? '').trim());
				break;
	...
	private async handleLocalTranscript(transcript: string): Promise<void> {
		if (!transcript || !this.document || !this.context) return;
		if (import.meta.env.DEV) console.debug('[local voice] transcript', transcript);
		this.turnChannel = 'voice';
		this.messages.push({
			id: crypto.randomUUID(),
			role: 'user',
		...
		try {
			...
			if (!llmState.installed) {
				throw new Error(
					`Local model required — download/install model: ${llmState.selectedModel.label} (${llmState.selectedModel.sizeMb} MB).`
				);
			}
			const host = getLlmHost({
				model: llmState.selectedModelId,
				dtype: llmState.selectedModel.dtype,
				device: 'webgpu',
				onProgress: (progress) => {
					if (import.meta.env.DEV) console.debug('[local voice] LLM progress', progress.status, progress.progress);
				}
			});
			if (import.meta.env.DEV) console.debug('[local voice] LLM started');
			const itemId = crypto.randomUUID();
			const response = await host.generate(
				[
					{ role: 'system', content: this.context.instructions },
					...this.messages.slice(-8).map((message) => ({
						role: message.role,
				...
				],
				{
					maxNewTokens: 192,
					temperature: 0.2,
					onToken: (token) => this.streamAssistantText(itemId, token, 'voice')
				}
			);
			if (import.meta.env.DEV) console.debug('[local voice] LLM completed', response.length);
			if (!response.trim()) throw new Error('The local model returned an empty answer.');
			this.settleTranscript();
			await this.speakLocalResponse(response);
		} catch (error) {
			this.activity = '';
			this.errorMessage = error instanceof Error ? error.message : 'The local voice response failed.';
			this.status = 'error';
			if (import.meta.env.DEV) console.debug('[local voice] response failed', error);
		}
	}

	private async speakLocalResponse(text: string): Promise<void> {
		if (!appState.installedModels.includes(appState.selectedModelId)) {
			throw new Error(
				`Local speech model required — install ${appState.selectedModelId} in Settings → Voice.`
			);
		}
		if (import.meta.env.DEV) console.debug('[local voice] TTS started');
		this.speaking = true;
		const hasArabic = /[\u0600-\u06ff]/u.test(text);
		const result =
			hasArabic && this.channel && 'synthesize' in this.channel
				? await this.channel.synthesize(text)
				: await ttsClient.synthesize(text, appState.selectedVoiceId);
		if (import.meta.env.DEV) console.debug('[local voice] TTS completed', result.audio.length);
		const context = new AudioContext();
		...
		source.onended = () => {
			...
			this.activity = '';
			void context.close().catch(() => undefined);
			if (this.localAudioContext === context) {
				this.localAudioContext = undefined;
				this.localAudioSource = undefined;
			}
			if (import.meta.env.DEV) console.debug('[local voice] audio playback finished');
		};
		this.localAudioContext = context;
		this.localAudioSource = source;
		if (import.meta.env.DEV) console.debug('[local voice] audio playback started');
		source.start();
	}

	private stopLocalPlayback(): void {
		try {
			this.localAudioSource?.stop();
		} catch {
			// The source may already have ended.
		}
		this.localAudioSource?.disconnect();
		void this.localAudioContext?.close().catch(() => undefined);
		this.localAudioSource = undefined;
		this.localAudioContext = undefined;
		this.speaking = false;
		if (import.meta.env.DEV) console.debug('[local voice] audio playback cancelled');
	}
