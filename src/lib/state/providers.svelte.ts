...
export class ProvidersState {
	...
	voiceProvider = $state<LocalVoiceProviderMode>('local');
	...
	private async load(): Promise<void> {
		try {
			...
			const [
				...
				getSetting<string>('voice-provider', 'local'),
			...
			this.voiceProvider = normalizeLocalVoiceProvider(voiceProvider);
	...
	async setVoiceProvider(provider: LocalVoiceProviderMode): Promise<void> {
		this.voiceProvider = normalizeLocalVoiceProvider(provider);
		await setSetting('voice-provider', this.voiceProvider);
	}

	get localVoiceUrl(): string {
		return buildLocalVoiceUrl();
	}
