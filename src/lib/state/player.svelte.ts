...
export class VoicebookPlayer {
	...
	async warmEngine(): Promise<void> {
		...
		try {
			...
			// Prepare the current passage before the user presses Play. The browser
			// still needs the click to unlock output, but model loading and first
			// synthesis no longer sit on the click path.
			if (this.book && this.currentSegment) {
				void this.queuedAudio(this.currentSegmentIndex).catch(() => undefined);
			}
		} catch {
	...
	private async prepareAudio(index: number): Promise<PreparedAudio> {
		...
		if (stored) {
			...
			try {
				...
				// A cache entry from an older codec/model must not block playback.
	...
	private async cacheSegment(index: number): Promise<void> {
		...
		if (playbackRequest) {
			...
			throw new Error('MUBSIR - مبصر generated a passage but could not save it on this device.');
		}
