...
import { deleteLlmModelAssets, hasLlmModelAssets } from '$lib/services/llm/llm-cache';
...
export class LlmState {
	...
	private async load(): Promise<void> {
		try {
			...
			const knownInstalled = installed.filter((id) => getLlmModel(id) !== null);
			const physicallyInstalled = (
				await Promise.all(
					LLM_CATALOG.map(async (model) =>
						(await hasLlmModelAssets(model.id)) ? model.id : null
					)
				)
			...
			if (import.meta.env.DEV) {
				try {
					localModelReady = Boolean(
						(await (await fetch('/local-models/manifest.json')).json()).llmReady
					);
				} catch {
					localModelReady = false;
				}
			}
			if (localModelReady && !physicallyInstalled.includes(DEFAULT_LLM_ID)) {
				physicallyInstalled.push(DEFAULT_LLM_ID);
			}
			this.installedModels = [...new Set([...knownInstalled, ...physicallyInstalled])].filter(
				(id) => physicallyInstalled.includes(id)
			);
			...
			if (this.installedModels.join('|') !== knownInstalled.join('|')) {
