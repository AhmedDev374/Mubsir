...
if (import.meta.env.DEV) env.localModelPath = '/local-models/llm/';
...
async function ensureModel(
	...
	initPromise = (async () => {
		const source = import.meta.env.DEV ? 'LFM2.5-1.2B-Instruct-ONNX' : model;
		generator = (await pipeline('text-generation', source, {
			device,
