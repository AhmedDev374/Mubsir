...
export class SupertonicAdapter {
	...
	private assetUrl(path: string): string {
		if (import.meta.env.DEV)
			return new URL(
				`/local-models/tts/Supertone-supertonic-3/${path}`,
	...
	private async response(path: string): Promise<Response> {
		...
		if (import.meta.env.DEV) {
			...
			if (!response.ok) throw new Error(`Could not load local ${path} (${response.status}).`);
			return response;
		}
	...
	private async bytes(
		...
		if (import.meta.env.DEV) {
			...
			progress({
				status: 'loading',
				file: path,
				progress: ((fileIndex + 1) / fileCount) * 100,
