...
export const DATABASE_LOCKED_MESSAGE =
	'MUBSIR - مبصر’s local storage is locked by another window. Close every other ' +
	'MUBSIR - مبصر tab and window (including the installed app, if any), then reload ' +
	'this page.';
...
export async function deleteAudio(key: string): Promise<void> {
	...
	if (record.path) await removeOpfs(record.path);
}
