...
import { containsArabic, normalizeArabicText } from './text-direction';
import {
	assemblePages,
	assignPageAnchors,
	blanketBoldShare,
...
// pages, embedded images, and bookmarks; v16 normalizes Arabic PDF text and
// uses geometry-aware RTL extraction when LiteParse returns visual-order runs.
export const DOCUMENT_NORMALIZATION_VERSION = 16;
...
function runsInBrowserWorker(): boolean {
...
export interface ImportOptions {
	...
	/** Reuse a fingerprint computed before dispatching work to an import worker. */
	fingerprint?: string;
}
...
export function kindForFile(file: Pick<File, 'name' | 'type'>): DocumentKind {
	...
	throw new ImportError(
		'MUBSIR - مبصر currently supports PDF, DOCX, Markdown, and plain text files.',
		'unsupported'
	);
}
...
export function pageLines(
	...
	const lineText = (line: PdfLine[]) =>
		line
			.sort((left, right) =>
				containsArabic(line.map((item) => item.text).join(' '))
					? right.x - left.x
					: left.x - right.x
			)
			.map((item) => item.text)
...
async function pdfDocumentExtras(
	...
	try {
		const pdfjs =
			typeof window === 'undefined' && !runsInBrowserWorker()
				? await import('pdfjs-dist/legacy/build/pdf.mjs')
				: await import('pdfjs-dist');
		if (typeof window !== 'undefined' || runsInBrowserWorker()) {
...
async function parsePdfWithLiteparse(
	...
	try {
		...
		try {
			...
			try {
				const complexity = (await probe.isComplex(data)) as PageComplexity[];
				pageCount = complexity.length;
				ocrPages = pagesNeedingOcr(complexity);
			} finally {
				probe.free();
			}
		} catch {
		...
		const parsedPages: LiteparsePage[] = [];
		const parsedImages: LiteparseImage[] = [];
		const chunkSize = 8;
		const chunks = pageCount
			? Array.from({ length: Math.ceil(pageCount / chunkSize) }, (_, index) => {
		...
		for (const chunk of chunks) {
			const parser = new glue.LiteParse({
				...
				quiet: true,
				...(chunk.end ? { targetPages: `${chunk.start}-${chunk.end}` } : {})
			});
			try {
				const result = await parser.parse(data);
				parsedPages.push(...result.pages);
				parsedImages.push(...result.images);
			} finally {
				parser.free();
			}
			if (pageCount) options.onProgress?.({ stage: 'parsing', page: chunk.end, pageCount });
		}

		const warnings: string[] = [];
		let ocrText = new Map<number, string>();
		if (ocrPages.length && options.enableOcr !== false && typeof OffscreenCanvas !== 'undefined') {
		...
		const usableMarkdown = liteparsePageMarkdown(withRecognizedPages(parsedPages, ocrText));
		if (pdfLooksScanned(usableMarkdown, parsedPages.length)) {
			throw new ImportError(SCANNED_PDF_MESSAGE, 'scanned-pdf');
		}
		// LiteParse can preserve Arabic glyphs while returning native RTL runs in
		// visual order. The pdf.js fallback below rebuilds those lines from their
		// coordinates without reversing the stored Unicode string.
		if (containsArabic(usableMarkdown) && !ocrPages.length) return null;
		const parsed = parsedSourceFromLiteparse({ pages: parsedPages, images: parsedImages }, ocrText);
...
async function parsePdf(file: File, options: ImportOptions = {}): Promise<ParsedSource> {
	...
	try {
		...
		for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
			...
			page.cleanup();
			options.onProgress?.({ stage: 'parsing', page: pageNumber, pageCount: pdf.numPages });
		}
...
export async function importFile(
	...
	return {
		...
		id,
		fingerprint: options.fingerprint ?? (await fingerprint(file)),
